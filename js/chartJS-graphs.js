...
const myChart1 = new Chart(ctx, {
	...
	,plugins: {
            title: {
                display: true,
                text: 'Registration of '+ carnet["detections"][0]["mmg"][0]["make_name"] +' '+ carnet["detections"][0]["mmg"][0]["model_name"] +' in ' + byCountry[0].year
            }
            ,legend: {
                display: false,
 	  }}
...
const myChart1 = new Chart(ctx, {
	...
	,plugins: {
            title: {
                display: true,
                text: 'Registration of '+ carnet["detections"][0]["mmg"][0]["make_name"] +' '+ carnet["detections"][0]["mmg"][0]["model_name"] +' by year'
            }
            ,legend: {
                display: false,
 	  }}
...
const Mh =byManufacturer.map(({Mh:actualValue})=>actualValue);
const avgEWLTP = byManufacturer.map(({avgEWLTP:actualValue})=>actualValue);
const target = Array(length.avgEwltp).fill(95);
console.log(target);
const ctx = document.getElementById('myChart');
const myChart1 = new Chart(ctx, {
    type: 'bar',
	...
	,plugins: {
            title: {
                display: true,
                text: 'Average Ewltp  of manufacturer(s) in EU27 by year'
            }
            ,legend: {
                display: false,
 	  }}
