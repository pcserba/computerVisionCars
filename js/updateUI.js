function showData() {
	//update the text
	//build the string
	txt = 'We detected a <B>' + results[0].Mh + ' </B> <B>'+results[0].Cn_short+'</B> with <B>99%</B> confidence.<P><B>Registrations in Europe ('+discomap1Result.results[0].year+'):</B> '+results[0].countVIN+' vehicles<P><B>Average specific CO<sub>2</sub> Emissions</B>: '+results[0].avgEWLTP+' g/km<P><B>Average engine capacity:</B> '+results[0].avgEc+'cm<sup>3</sup><P><B>Average engine power:</B> '+results[0].avgEp+'kwh<P><B>Average mass in running order:</B> '+results[0].avgM+'kg';
